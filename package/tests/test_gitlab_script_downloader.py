...
class TestGitLabScriptDownloader(TestCase):
    ...
    def test_validate_gitlab_url_passes(self):
        # arrange
        gitlab_downloader = GitLabScriptDownloader(Mock(), Mock())
        ...
        # act
        gitlab_downloader._validate_gitlab_url(url1)
        gitlab_downloader._validate_gitlab_url(url2)
        gitlab_downloader._validate_gitlab_url(url3)
        gitlab_downloader._validate_gitlab_url(url4)
    ...
    def test_extract_data_from_url_with_ref(self):
        ...
        # act
        data = gitlab_downloader._extract_data_from_url(url1)
    ...
    def test_is_url_to_file_returns_true_with_folders(self):
        # arrange
        gitlab_downloader = GitLabScriptDownloader(Mock(), ALLOWED_FILES_PATTERN)
        file_path = "folder1/some_file.ps1"

        # act
        result = gitlab_downloader.is_url_to_file(file_path)
    ...
    def test_build_url_to_list_files_in_path_with_ref_and_folder(self):
        ...
        # act
        result = gitlab_downloader._build_url_to_list_files_in_path(url_data)
    ...
    def test_remove_junk_files_from_files_list(self):
        ...
        # act
        approved_list = gitlab_downloader._remove_junk_files_and_folders_from_files_list(files_list)
    ...
    def test_get_main_file_ps1(self):
        ...
        # act
        main = gitlab_downloader.get_main_file(files_list)
    ...
    def test_get_main_file_raises_no_main(self):
        ...
        with self.assertRaisesRegexp(Exception, "Main file doesn't exist"):
            gitlab_downloader.get_main_file(files_list)
    ...
    def test_download_single_file(self):
        ...
        file_name = Mock()
        file_txt = Mock()
        gitlab_downloader._download_single_file = Mock(return_value=(file_name, file_txt))
        url = 'https://gitlab.com/api/v4/projects/21915601/repository/files/test_dir/bla.sh?ref=master'

        # act
        result = gitlab_downloader.download(url, Mock())
    ...
    def test_download_single_dir(self):
        ...
        gitlab_downloader._get_files_in_path = Mock()
        gitlab_downloader._remove_junk_files_and_folders_from_files_list = Mock(return_value=files_list_in_path)
        gitlab_downloader.get_main_file = Mock(return_value=main_file_info)
        gitlab_downloader._build_url_to_download_single_file = Mock()
        ...
        gitlab_downloader._build_url_to_download_single_file.assert_has_calls([call(ANY, files_list_in_path[0]),
    ...
    def test_integration_download_folder(self):
        ...
        gitlab_downloader.download('https://gitlab.com/api/v4/projects/21915601/repository/files/test_dir', auth)
